...
usage()
{
    ...
    echo "   -u [branch]    Update git repo and update rust (run in redox/ repo only)."
    echo "                  If blank 'branch' defaults to master"
    echo "   -s             Check the status of the current CI build"
...
rustInstall() {
    ...
    if [ -z "$(which rustup)" ]; then
        ...
        if echo "$rustup" | grep -iq "^y" ;then
            echo "Installing rustup with default toolchain nightly..."
            # run rustup install script
            curl https://sh.rustup.rs -sSf | sh -s -- --default-toolchain nightly
            # You have to add the rustup variables to the $PATH
            # Please NEVER export ~/.cargo/bin before $PATH, it's a huge security issue. ~W
